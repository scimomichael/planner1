...
let _pendingImage=null;let _rec=null;let _recActive=false;
function attachImage(file){if(!file)return;const img=new Image();const url=URL.createObjectURL(file);img.onload=()=>{const MAX=1568;let w=img.width,h=img.height;if(Math.max(w,h)>MAX){const k=MAX/Math.max(w,h);w=Math.round(w*k);h=Math.round(h*k);}const cv=document.createElement('canvas');cv.width=w;cv.height=h;cv.getContext('2d').drawImage(img,0,0,w,h);const dataUrl=cv.toDataURL('image/jpeg',0.85);_pendingImage={media_type:'image/jpeg',data:dataUrl.split(',')[1],preview:dataUrl};URL.revokeObjectURL(url);const pv=document.getElementById('aiAttachPreview');const th=document.getElementById('aiAttachThumb');if(pv&&th){th.src=dataUrl;pv.style.display='flex';}};img.onerror=()=>{URL.revokeObjectURL(url);Store.toast('Could not read that image');};img.src=url;}
function clearAttachment(){_pendingImage=null;const pv=document.getElementById('aiAttachPreview');if(pv)pv.style.display='none';}
...
async function send(){const input=document.getElementById('aiInput');const text=input.value.trim();const img=_pendingImage;if(!text&&!img)return;input.value='';input.style.height='auto';clearAttachment();history.push({role:'user',content:text||'(screenshot attached)',image:img?img.preview:undefined});renderMessages();const sendBtn=document.querySelector('.ai-send');if(sendBtn)sendBtn.disabled=true;let typing=_typingEl();try{const ctx=_buildContext();let msgs=history.filter(m=>m.role==='user'||m.role==='assistant').slice(-12).map(m=>({role:m.role,content:m.content}));if(img&&msgs.length){const last=msgs[msgs.length-1];last.content=[{type:'image',source:{type:'base64',media_type:img.media_type,data:img.data}},{type:'text',text:text||'Here is a screenshot. Add anything schedulable from it to my planner.'}];}let data=await _callChat(msgs,ctx);
