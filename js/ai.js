const AI=(()=>{let history=[];const WELCOME={role:'assistant',content:"Hi Michael! Tell me what to add, move, or find. What type of block, what day and time, which class, and any details you want included."};function init(){history=[WELCOME];renderMessages();const bubble=document.getElementById('aiBubble');if(bubble)bubble.style.display=Settings.get('sAIEnabled',true)?'':'none';}function _clearHistory(){history=[WELCOME];renderMessages();}function toggle(){const panel=document.getElementById('aiPanel');const wasOpen=panel.classList.contains('open');_clearHistory();panel.classList.toggle('open');if(!wasOpen){setTimeout(()=>{const body=document.getElementById('aiMessages');if(body)body.scrollTop=body.scrollHeight;const input=document.getElementById('aiInput');if(input)input.focus();},100);}}function renderMessages(){const body=document.getElementById('aiMessages');if(!body)return;body.innerHTML=history.map(m=>{if(m.role==='user')return`<div class="ai-msg user">${m.images&&m.images.length?`<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:6px">`+m.images.map(u=>`<img src="${u}" style="max-width:110px;border-radius:8px" alt="screenshot">`).join('')+`</div>`:''}${Store.esc(m.content)}</div>`;const actionNote=m.actionsApplied?`<div class="ai-msg-actions"><strong>\u2713 Applied ${m.actionsApplied} change${m.actionsApplied===1?'':'s'}</strong> to your planner</div>`:'';return`<div class="ai-msg assistant">${_formatMarkdown(m.content)}${actionNote}</div>`;}).join('');body.scrollTop=body.scrollHeight;}function _formatMarkdown(text){let t=Store.esc(text);t=t.replace(/```(\w*)\n([\s\S]*?)```/g,'<pre style="font-family:var(--mono);background:rgba(0,0,0,.06);padding:8px 10px;border-radius:6px;font-size:.85em;overflow-x:auto;margin:4px 0">$2</pre>');t=t.replace(/\*\*(.+?)\*\*/g,'<strong>$1</strong>');t=t.replace(/\*(.+?)\*/g,'<em>$1</em>');t=t.replace(/`(.+?)`/g,'<code style="font-family:var(--mono);background:rgba(0,0,0,.06);padding:1px 5px;border-radius:4px;font-size:.9em">$1</code>');t=t.replace(/\n/g,'<br>');return t;}function handleKey(e){if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send();}}function _typingEl(){const body=document.getElementById('aiMessages');const t=document.createElement('div');t.className='ai-msg typing';t.innerHTML='<div class="ai-typing-dot"></div><div class="ai-typing-dot"></div><div class="ai-typing-dot"></div>';body.appendChild(t);body.scrollTop=body.scrollHeight;return t;}
async function _callChat(msgs,ctx){const res=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:msgs,context:ctx})});if(!res.ok){const err=await res.json().catch(()=>({error:'Unknown error'}));throw new Error((err.error||'Something went wrong')+'. Make sure your Anthropic API key is set in Netlify env vars as ANTHROPIC_API_KEY.');}return res.json();}
let _pendingImages=[];let _rec=null;let _recActive=false;const MAX_ATTACH=6;
function attachImages(fileList){const files=[...(fileList||[])].filter(f=>f&&f.type&&f.type.startsWith('image/'));if(!files.length)return;const room=MAX_ATTACH-_pendingImages.length;if(room<=0){Store.toast('Max '+MAX_ATTACH+' screenshots per message');return;}
files.slice(0,room).forEach(file=>{const img=new Image();const url=URL.createObjectURL(file);img.onload=()=>{const MAX=1568;let w=img.width,h=img.height;if(Math.max(w,h)>MAX){const k=MAX/Math.max(w,h);w=Math.round(w*k);h=Math.round(h*k);}const cv=document.createElement('canvas');cv.width=w;cv.height=h;cv.getContext('2d').drawImage(img,0,0,w,h);const dataUrl=cv.toDataURL('image/jpeg',0.85);_pendingImages.push({media_type:'image/jpeg',data:dataUrl.split(',')[1],preview:dataUrl});URL.revokeObjectURL(url);_renderAttachPreview();};img.onerror=()=>{URL.revokeObjectURL(url);Store.toast('Could not read an image');};img.src=url;});
if(files.length>room)Store.toast('Only added '+room+' (max '+MAX_ATTACH+' per message)');}
function removeAttachment(i){_pendingImages.splice(i,1);_renderAttachPreview();}
function clearAttachment(){_pendingImages=[];_renderAttachPreview();}
function _renderAttachPreview(){const pv=document.getElementById('aiAttachPreview');if(!pv)return;if(!_pendingImages.length){pv.style.display='none';pv.innerHTML='';return;}pv.style.display='flex';pv.innerHTML=_pendingImages.map((im,i)=>`<span style="position:relative;display:inline-block"><img src="${im.preview}" style="height:44px;border-radius:8px;border:1px solid var(--line)" alt="attachment ${i+1}"><button class="btn-icon" onclick="AI.removeAttachment(${i})" title="Remove" style="position:absolute;top:-6px;right:-6px;font-size:10px;background:var(--surface);border:1px solid var(--line);border-radius:50%;width:16px;height:16px;line-height:1;padding:0">\u2715</button></span>`).join('');}
function toggleVoice(){const btn=document.getElementById('aiMicBtn');const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){Store.toast('Voice input is not supported in this browser');return;}
if(_recActive){try{_rec.stop();}catch{}return;}
_rec=new SR();_rec.lang='en-US';_rec.interimResults=true;_rec.continuous=true;let base=document.getElementById('aiInput').value;
_rec.onresult=e=>{let final='',interim='';for(let i=0;i<e.results.length;i++){const r=e.results[i];if(r.isFinal)final+=r[0].transcript;else interim+=r[0].transcript;}const inp=document.getElementById('aiInput');inp.value=(base+' '+final+' '+interim).replace(/\s+/g,' ').trim();inp.style.height='auto';inp.style.height=inp.scrollHeight+'px';};
_rec.onend=()=>{_recActive=false;if(btn)btn.classList.remove('recording');};
_rec.onerror=ev=>{_recActive=false;if(btn)btn.classList.remove('recording');if(ev.error==='not-allowed')Store.toast('Microphone access was denied');};
_recActive=true;if(btn)btn.classList.add('recording');_rec.start();}
async function send(){const input=document.getElementById('aiInput');const text=input.value.trim();const imgs=_pendingImages.slice();if(!text&&!imgs.length)return;input.value='';input.style.height='auto';clearAttachment();history.push({role:'user',content:text||('('+imgs.length+' screenshot'+(imgs.length===1?'':'s')+' attached)'),images:imgs.length?imgs.map(im=>im.preview):undefined});renderMessages();const sendBtn=document.querySelector('.ai-send');if(sendBtn)sendBtn.disabled=true;let typing=_typingEl();try{const ctx=_buildContext();let msgs=history.filter(m=>m.role==='user'||m.role==='assistant').slice(-12).map(m=>({role:m.role,content:m.content}));if(imgs.length&&msgs.length){const last=msgs[msgs.length-1];last.content=[...imgs.map(im=>({type:'image',source:{type:'base64',media_type:im.media_type,data:im.data}})),{type:'text',text:text||('Here '+(imgs.length===1?'is a screenshot':'are '+imgs.length+' screenshots')+'. Add anything schedulable from them to my planner.')}];}let data=await _callChat(msgs,ctx);
// Tool-use round trip: the model calls the search_email tool; Gmail lives in
// this browser, so we execute the search here, return tool_result blocks,
// and let the model continue. Up to 3 rounds.
let rounds=0;
while(data&&Array.isArray(data.emailToolCalls)&&data.emailToolCalls.length&&rounds<3){rounds++;
if(data.text){history.push({role:'assistant',content:data.text});renderMessages();}
const results=[];
for(const call of data.emailToolCalls){let resultStr='';
if(typeof Gmail==='undefined'||!Gmail.isConnected()){resultStr='ERROR: Gmail is not connected. Tell the user to open Settings, paste their Google OAuth Client ID under Gmail, and tap Connect, then ask again.';}
else{try{const q=String((call.input&&call.input.query)||'');const found=await Gmail.search(q,call.input&&call.input.maxResults);if(!found.length)resultStr='No messages matched query: "'+q+'"';else resultStr=found.map((m,idx)=>'[Message '+(idx+1)+']\nFrom: '+m.from+'\nSubject: '+m.subject+'\nDate: '+m.date+'\nBody:\n'+m.body).join('\n\n');}catch(err){resultStr='ERROR: '+(err.message||err);}}
results.push({type:'tool_result',tool_use_id:call.id,content:resultStr});}
msgs=[...msgs,{role:'assistant',content:data.assistantContent},{role:'user',content:results}];
data=await _callChat(msgs,ctx);}
typing.remove();typing=null;
const acts=Array.isArray(data.actions)?data.actions.filter(a=>a&&a.type!=='search_email'):[];
const actionsApplied=acts.length?executeActions(acts):0;
history.push({role:'assistant',content:data.text||'(no response)',actionsApplied});renderMessages();if(actionsApplied>0)App.refresh();}catch(err){if(typing)typing.remove();history.push({role:'assistant',content:`\u26a0\ufe0f ${err.message||err}`});renderMessages();}finally{if(sendBtn)sendBtn.disabled=false;}}function _buildContext(){
