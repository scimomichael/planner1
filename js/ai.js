...
async function send(){const input=document.getElementById('aiInput');const text=input.value.trim();if(!text)return;input.value='';input.style.height='auto';history.push({role:'user',content:text});renderMessages();const sendBtn=document.querySelector('.ai-send');if(sendBtn)sendBtn.disabled=true;let typing=_typingEl();try{const ctx=_buildContext();const msgsForApi=history.filter(m=>m.role==='user'||m.role==='assistant').slice(-12).map(m=>({role:m.role,content:m.content}));const data=await _callChat(msgsForApi,ctx);typing.remove();typing=null;const allActs=Array.isArray(data.actions)?data.actions:[];const searches=allActs.filter(a=>a&&a.type==='search_email');const otherActs=allActs.filter(a=>a&&a.type!=='search_email');
// Email-search agentic loop: run Gmail searches in-browser, hand results
// back to the model in a follow-up turn, then execute its final actions.
if(searches.length){if(typeof Gmail==='undefined'||!Gmail.isConnected()){const applied=otherActs.length?executeActions(otherActs):0;history.push({role:'assistant',content:(data.text?data.text+'\n\n':'')+'Gmail is not connected. Open Settings, paste your Google OAuth Client ID under Gmail, and tap Connect. Then ask me again.',actionsApplied:applied});renderMessages();if(applied>0)App.refresh();return;}
if(data.text){history.push({role:'assistant',content:data.text});renderMessages();}
const preApplied=otherActs.length?executeActions(otherActs):0;
let resultsText='EMAIL SEARCH RESULTS (fetched from the user\'s Gmail in their browser). Extract real event details from these and emit planner actions. Never invent details that are not in the emails. If nothing relevant is here, say so.\n';
for(const s of searches){try{const found=await Gmail.search(String(s.query||''),s.maxResults);resultsText+='\nQuery: "'+(s.query||'')+'" -> '+found.length+' message(s)\n';found.forEach((m,idx)=>{resultsText+='\n[Message '+(idx+1)+']\nFrom: '+m.from+'\nSubject: '+m.subject+'\nDate: '+m.date+'\nBody:\n'+m.body+'\n';});}catch(err){resultsText+='\nQuery: "'+(s.query||'')+'" -> ERROR: '+(err.message||err)+'\n';}}
const followMsgs=[...msgsForApi,{role:'assistant',content:data.text||'(searching email)'},{role:'user',content:resultsText}];
typing=_typingEl();const data2=await _callChat(followMsgs,ctx);typing.remove();typing=null;
const acts2=(Array.isArray(data2.actions)?data2.actions:[]).filter(a=>a&&a.type!=='search_email');
const applied2=acts2.length?executeActions(acts2):0;
history.push({role:'assistant',content:data2.text||'(no response)',actionsApplied:applied2});renderMessages();if(applied2>0||preApplied>0)App.refresh();return;}
const actionsApplied=otherActs.length?executeActions(otherActs):0;history.push({role:'assistant',content:data.text||'(no response)',actionsApplied});renderMessages();if(actionsApplied>0)App.refresh();}catch(err){if(typing)typing.remove();history.push({role:'assistant',content:`\u26a0\ufe0f ${err.message||err}`});renderMessages();}finally{if(sendBtn)sendBtn.disabled=false;}}function _buildContext(){
